...
RSpec.describe Api::V1::SubSectorsController, type: :routing do
  ...
  describe 'routing' do
    ...
    it 'routes to #show' do
      expect(
        get: "/api/v1/main_sectors/#{main_sector.id}/sub_sectors/#{sub_sector.id}"
      ).to route_to(
    ...
    it 'routes to #update via PUT' do
      expect(
        put: "/api/v1/main_sectors/#{main_sector.id}/sub_sectors/#{sub_sector.id}"
      ).to route_to(
    ...
    it 'routes to #update via PATCH' do
      expect(
        patch: "/api/v1/main_sectors/#{main_sector.id}/sub_sectors/#{sub_sector.id}"
      ).to route_to(
        controller: 'api/v1/sub_sectors', action: 'update',
        main_sector_id: main_sector.id.to_s, id: sub_sector.id.to_s
      )
    ...
    it 'routes to #destroy' do
      expect(
        delete: "/api/v1/main_sectors/#{main_sector.id}/sub_sectors/#{sub_sector.id}"
      ).to route_to(
